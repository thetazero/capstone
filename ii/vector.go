...
//ComplexCompute computes the complex value of the vector at complex x as if it is a polynomial, ie: [1,2,3]=1+2x+3x^2
func (v Vector) ComplexCompute(x Complex) Complex {
	c := Complex{big.NewRat(1, 1), big.NewRat(0, 1)}
	ans := Complex{big.NewRat(0, 1), big.NewRat(0, 1)}
	for i := range v {
		ans.Add(new(Complex).Mul(c, Complex{v[i], big.NewRat(0, 1)}), ans)
		c.Mul(c, x)
	}
	return ans
}
